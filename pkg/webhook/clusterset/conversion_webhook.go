...
func convertExampleCRD(Object *unstructured.Unstructured, toVersion string) (*unstructured.Unstructured, metav1.Status) {
	...
	case "cluster.open-cluster-management.io/v1beta2":
		switch toVersion {
		case "cluster.open-cluster-management.io/v1beta1":
			selectorType, found, err := unstructured.NestedString(convertedObject.Object, "spec", "clusterSelector", "selectorType")
			klog.Errorf("#######1 type: %v, err: %v", selectorType, err)

			if err != nil || !found {
				return nil, statusErrorWithMessage("can not find selectortype %q", err)
			}
			if selectorType == string(clusterv1beta2.ExclusiveClusterSetLabel) {
				err = unstructured.SetNestedField(convertedObject.Object, clusterv1beta1.LegacyClusterSetLabel, "spec", "clusterSelector", "selectorType")
			}
		default:
	...
	case "cluster.open-cluster-management.io/v1beta1":
		...
		case "cluster.open-cluster-management.io/v1beta2":
			selectorType, found, err := unstructured.NestedString(convertedObject.Object, "spec", "clusterSelector", "selectorType")
			klog.Errorf("#######2 type: %v, err: %v", selectorType, err)

			if err != nil || !found {
				return nil, statusErrorWithMessage("can not find selectortype %q", err)
			}

			if selectorType == string(clusterv1beta1.LegacyClusterSetLabel) {
				err = unstructured.SetNestedField(convertedObject.Object, "ExclusiveClusterSetLabel", "spec", "clusterSelector", "selectorType")
				klog.Errorf("##### %v", err)
			}
		default:
			//return nil, statusErrorWithMessage("unexpected conversion version %q", toVersion)
